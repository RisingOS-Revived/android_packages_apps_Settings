...
public class BatteryChargeSoundSettings extends SettingsPreferenceFragment implements
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // Using try-catch to handle potential ClassCastException
        try {
            ...
            // If we can't cast to CustomSeekBarPreference, try using standard SeekBarPreference instead
            Log.e("BatteryChargeSound", "Failed to find CustomSeekBarPreference, falling back to SeekBarPreference", e);
            SeekBarPreference seekBar = (SeekBarPreference) findPreference(KEY_BATTERY_LEVEL_CHARGE_SEEK_BAR);
            if (seekBar != null) {
                ...
                // Replace the custom one with standard one in preference list for consistent UI
                getPreferenceScreen().removePreference(findPreference(KEY_BATTERY_LEVEL_CHARGE_SEEK_BAR));
                getPreferenceScreen().addPreference(seekBar);
            }
        }
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if (preference.equals(mBatteryLevelChargeAlarmEnabled)) {
            ...
            return true;
        } else if (preference instanceof CustomSeekBarPreference && preference.getKey().equals(KEY_BATTERY_LEVEL_CHARGE_SEEK_BAR)) {
            int val = (Integer) newValue;
            Settings.System.putIntForUser(getContentResolver(),
                    Settings.System.SEEK_BAR_BATTERY_CHARGE_LEVEL_SOUND, val, UserHandle.USER_CURRENT);
            return true;
        } else if (preference instanceof SeekBarPreference && preference.getKey().equals(KEY_BATTERY_LEVEL_CHARGE_SEEK_BAR)) {
            // Handle standard SeekBarPreference if we're using it as fallback
            int val = (Integer) newValue;
            Settings.System.putIntForUser(getContentResolver(),
                    Settings.System.SEEK_BAR_BATTERY_CHARGE_LEVEL_SOUND, val, UserHandle.USER_CURRENT);
            return true;
